...
namespace BenBailey_InventorySystem.Forms
{
    public partial class ModifyProductScreen : Form
    {
        ...
        /// <summary>
        /// Loads form with Product data and creates DataGridViews
        /// </summary>
        /// <param name="product"></param>
        private void LoadProduct(Product product)
        {
            prodID.Text = product.ProductID.ToString();
            //prodID.Enabled = false;
            prodName.Text = product.Name;
            prodInventory.Text = product.Inventory.ToString();
            prodPrice.Text = product.Price.ToString();
            prodMax.Text = product.Max.ToString();
            prodMin.Text = product.Min.ToString();
            ...
            dataGridAllParts.Columns["PartID"].Name = "PartID";
            dataGridAllParts.Columns["Name"].Name = "Name";
            ...
            var assocPartsBS = new BindingSource();
            assocPartsBS.DataSource = assocParts;
            dataGridAssocParts.DataSource = assocPartsBS;
        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (this.prodPrice.Text.StartsWith("$"))
                {
                    prodPrice = decimal.Parse(this.prodPrice.Text.Substring(1));
                }
                else
                {
                    prodPrice = Convert.ToDecimal(this.prodPrice.Text);
                }
                ...
                BenBailey_InventorySystem.Models.Inventory.UpdateProduct(prodID, product);
                this.Close();
                mainScreen.refreshDataGrids();
            }
            catch (Exception ex)
            {
                MessageBox.Show("An error occured while saving the modifications. Please check your inputs and try again.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                throw ex;
            }
        ...
        private void buttonDelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Part currentPart = (Part)dataGridAssocParts.CurrentRow.DataBoundItem;
                    int prodID = Convert.ToInt32(this.prodID.Text);
                    Product product = BenBailey_InventorySystem.Models.Inventory.LookupProduct(prodID);
                    product.RemoveAssociatedPart(currentPart.PartID);

                    foreach (DataGridViewRow row in dataGridAssocParts.SelectedRows)
                    {
                        dataGridAssocParts.Rows.RemoveAt(row.Index);
                    }
                    dataGridAssocParts.Refresh();
                }
                else
                    return;
            }
        ...
        {
            DialogResult result = MessageBox.Show("Are you sure you want to cancel modifying this product? All unsaved changes will be lost.", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
